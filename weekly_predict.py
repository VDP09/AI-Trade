...
def prepare_ticker(ticker, macro_data):
    ...
    # Exclude last HORIZON rows from training — their targets are unknown.
...
def fill_past_actuals(df):
    """Downloads each ticker once and reuses across all rows."""
    filled = 0
    ticker_data_cache = {}
...
def execute_trades(predictions):
    ...
    for ticker in TICKERS:
        ...
        date_str = p["date"]
        if p["signal"] == "BUY":
            desired_sym, undesired_sym = info["long_etf"], info.get("short_etf")
        elif p["signal"] == "SHORT":
            desired_sym, undesired_sym = info["short_etf"], info["long_etf"]
        else:
            desired_sym, undesired_sym = None, info["long_etf"]
        try:
            ...
            if desired_sym:
                ...
                if buy_cid in open_order_ids:
                    ...
                    continue
                client.submit_order(MarketOrderRequest(
                    symbol=desired_sym, notional=round(per_ticker, 2), side=OrderSide.BUY,
                    time_in_force=TimeInForce.DAY,
